...
def only_passing(actions: np.ndarray) -> bool:
    """ Whether the given actions only contain the passing action.

    Args:
        actions (np.ndarray): The possible actions

    Returns:
        bool: Whether the actions only contain the passing action.
    """

    return len(actions) == 1 and np.all(actions[0] == 0)
